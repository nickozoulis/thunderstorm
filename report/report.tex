...
\begin{enumerate}
	\item Suppose incoming query K=5.
	\item If its new, put it in HBase so both layers start computing.
	\item If it isn't new,
	\begin{enumerate}
		\item Check if Stream has answer (Stream is fed views from Batch)
		\item If Stream hasn't,
			\begin{enumerate}
				\item Do a K-out-of-K' local K-Means, where K$<<$K' (eg. K'=10000)
			\end{enumerate}
	\end{enumerate}
\end{enumerate}
...
Following example commands to run the modules, all executed from the root project folder:
...
$ [1] java -jar data_stream_simulator/target/data_stream_simulator{..}jar-with-dependencies.jar -f /filepath_to_normalized_dataset/normalized_dataset
\end{lstlisting}
