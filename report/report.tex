...
% Redefine foreach loop that mimics forall loop
...
\begin{enumerate}
	\item Raw data: Stores the whole dataset, each row consisting a data point under a single column family, using a column foreach dimension. Rowkey is a long monotonically incrementing counter.
	\item Queries: Stores the users' queries. Rowkey is a long monotonically incrementing counter. The first row of this table is specially used to hold the maximum counter of the rows, which is used from both layers to locate the new incoming queries. The user's K-Means query is stored in two columns, a column for an Integer containing the K cluster number and a String in an other column containing the filters (both columns under the same column family).
	\item Batch views: Stores the results of the queries processed by the batch layer as views. Rowkey is a long number indicating the query in Queries table that corresponds to this view. There is one column family that contains as many columns as the query requires. As a column identifier is use a string prefix along with an Integer monotonically increasing counter.
...
\begin{lstlisting}
	...
	// uses two worker processes  
	conf.setNumWorkers(2); 
	
	// sets parallelism hint to 2 
...
The module is configured to add a specific range of rows (and dimensions of them) into HBase. More specifically we insert, from a file in local file system, 10 thousand rows every 90 seconds directly to HBase. Every row is added as an individual $Put$ hbase command and we use a monotonically incrementing counter as the rowkey. The frequency is fixed during the whole run of data stream simulation's module. The data added via this module to HBase are immutable.
...
Batch's view will take much longer to compute, but takes into account every data information for the first data input till the time point it started computing.This approach provides resilience and fault tolerance to corrupted incoming data, because these could be removed while batch would recompute the views in the next round and overwrite the faulty views. Also, because each time it recomputes everything from scratch,it leverages Batch layer to 'remember' every data point information. The downside, aside from the time complexity, is that from the point in time that Batch started computation till the actual current point, could happen data inputs of which Batch would be unaware of.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Function{Silhouette Coefficient}{$Point$ $p$, $Clusters$ $c$}
	...
	\begin{algorithmic}[1]
		...
		\Function{Silhouette Score}{$Dataset$ $d$, $Clusters$ $c$}
...
Silhouette experiments were illuminating in understanding the way different fusion schemes impact the quality of the clustering. Having the Batch 0\% - Stream 100\%, call it $[0]$, (essentially meaning that every layer acts on its own, without any knowledge 'feeding' from batch to stream) as a reference point enabled as to compare with both the Batch 50\% - Stream 50\%, call it $[1]$, and Batch 95\% - Stream 5\%, call it $[2]$ .
...
Clusterheads' visualizations informed us about the structure of the dataset and as it seems, while clusters begin with quite non-uniformly borders, they are smoothed at the end of each layer computations, meaning that the there is structure in our dataset thus clusters end to be uniform. 
...
\section{Fotios Avgerinos}

\section{Timotheos Constambeys}

This project was a great opportunity for me to learn a number of new things. I have learnt the fundamental aspects of big data and lamda architecture. In addition,  new tools that I have never heard before such as Apache Spark, Storm , HBase and various other software available from the community.  I have learnt new compiling and execution tools for complex projects such as the open source Apache Maven tool. Furthermore, working as a team and splitting a large project into smaller parts was a great experience.   

\section{Nickolas-Stylianos Zoulis}

Through the course of this project I have gained invaluable experience in a wide range of aspects. We were challenged with a research field where not lots of previous work has been done and had to make important decisions on which technologies should formulate our architecture, comparing their architectural designs and based on our system's use cases. We also had to understand and implement machine learning algorithms, metrics and experiments in order to measure how well our approach performed. Last but not least, the procedure of reporting our observations in a scientific manner was something new to me and tried to make the most use of it. I was in favor of the more technical approach in order to have the chance to get my hands under the hood of these big data systems to program, configure, make them work individually as well in collaboration. I am in the happy position to say that this project is one of my most novel software contributions so far.
...
\begin{appendices}
	\chapter{Serving layer command line manual}
	\begin{itemize}
	\item get [batch/stream] [qid] -> Gets the views of the input query ID from the specified table.
		\item kmeans [numOfClusters] (; [filter]) : Inserts a (constrained) KMeans query into HBase.
		\item put [absolute file path] : Inserts data to the raw data table.
		\item test : Tests the connection with HBase.
		\item scan [tableName] [options: 0,1,2] : Performs a scan of the input table.
		\item create : Create the schema tables of the HBase.
		\item man : Displays serving layer's manual.
